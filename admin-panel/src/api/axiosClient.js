  ...
  try {
    ...
    if (!skipApiCall) {
      try {
        ...
        console.warn(
          'Logout API call failed (this is expected if already unauthorized):',
          apiError.message,
        );
      }
    }
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      const isLogoutEndpoint =
        requestUrl.includes('/admin/logout') || requestUrl.includes('admin/logout');
      ...
      if (!isAuthEndpoint && !isLogoutEndpoint) {
        ...
        console.log(
          '401 error on auth endpoint (this is expected for invalid credentials):',
          requestUrl,
        );
      }
    }
