...
const dashboardApi = {
  ...
  getDashboardStats(params = {}) {
    ...
    if (hasTimeRange && (hasStartDate || hasEndDate)) {
      return Promise.reject(new Error("Cannot use time_range together with start_date/end_date. Use either time_range OR custom dates."));
    }

    // If using custom dates, both are required
    if ((hasStartDate && !hasEndDate) || (!hasStartDate && hasEndDate)) {
      return Promise.reject(new Error("Both start_date and end_date are required together when using custom date range."));
    }
    ...
    if (hasTimeRange) {
      ...
      if (!validTimeRanges.includes(params.time_range)) {
        return Promise.reject(new Error(`Invalid time_range. Must be one of: ${validTimeRanges.join(', ')}`));
      }
    }
