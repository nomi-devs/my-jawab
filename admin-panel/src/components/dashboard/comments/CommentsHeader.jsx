// src/components/dashboard/comments/CommentsHeader.jsx
import React, { useState, useEffect, useRef } from 'react';
import { useTranslation } from 'react-i18next';
import { MessageSquare, Search, X, Filter, Grid, List, Download } from 'lucide-react';
  ...
  ({
    ...
    const handleExportData = async () => {
      const params = {
        ...
        is_approved:
          selectedStatus === 'approved' ? true : selectedStatus === 'pending' ? false : undefined,
        sort_by: selectedSortBy,
        sort_order: selectedSortOrder,
      };

      try {
        const response = await commentsApi.exportComments(params);
      ...
      <div className="p-4 border-b border-purple-100 dark:border-gray-700 transition-colors">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 transition-colors">
              {loading ? (
                ...
                t('commentsHeader.totalComments', { count: commentCount })
              )}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-purple-50 dark:bg-gray-700 rounded-lg p-0.5">
              <button
                ...
                title={t('commentsHeader.listView')}
                disabled={loading}
              >
                <List
                  size={16}
              ...
              <button
                ...
                title={t('commentsHeader.gridView')}
                disabled={loading}
              >
                <Grid
                  size={16}
            ...
            {onRefresh && (
              <RefreshButton
                onClick={onRefresh}
                loading={loading}
                title={t('commentsHeader.refreshComments')}
                size={18}
              />
            )}
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
          ...
          <div className="relative flex-1 w-full sm:w-auto sm:min-w-[200px]">
            ...
            <input
              type="text"
              placeholder={t('commentsHeader.searchPlaceholder')}
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              <option value="all">{t('commentsHeader.allStatus')}</option>
              <option value="approved">{t('commentsHeader.approved')}</option>
              <option value="pending">{t('commentsHeader.pending')}</option>
            </select>
          </div>
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              <option value="created_at">{t('commentsHeader.sortDate')}</option>
              <option value="like_count">{t('commentsHeader.sortLikes')}</option>
            </select>
          </div>
          ...
          <button
            ...
            title={
              selectedSortOrder === 'DESC'
                ? t('commentsHeader.sortNewestFirst')
                : t('commentsHeader.sortOldestFirst')
            }
          >
            ...
            <span className="hidden sm:inline">
              {selectedSortOrder === 'DESC' ? t('commentsHeader.desc') : t('commentsHeader.asc')}
            </span>
          </button>
          ...
          <button
            ...
            title={hasActiveFilters ? t('commentsHeader.clearAllFilters') : t('commentsHeader.noFiltersToClear')}
          >
            <X size={14} />
            <span>{t('commentsHeader.clear')}</span>
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {localSearchTerm && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('commentsHeader.searchLabel')}</span>
                ...
                <button
                  ...
                  title={t('commentsHeader.removeSearch')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {selectedStatus !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('commentsHeader.statusLabel')}</span>
                ...
                <button
                  ...
                  className="text-purple-500 hover:text-purple-700 dark:hover:text-purple-200 ms-0.5 transition-colors"
                  title={t('commentsHeader.removeStatusFilter')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {(selectedSortBy !== 'created_at' || selectedSortOrder !== 'DESC') && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('commentsHeader.sortLabel')}</span>
