...
import {
    Sparkles, Infinity as InfinityIcon, Check, X,
    FileText, MessageSquare, Users, Hash, Video,
    BadgeCheck, Headphones, Rocket, EyeOff, User,
} from 'lucide-react';

// ─── Feature definitions ───────────────────────────────
// Each feature has: key, label, description, type, icon, (for numeric) unit
const BOOLEAN_FEATURES = [
    { key: 'can_create_polls', label: 'Create polls', icon: FileText, description: 'Allow creating polls' },
    { key: 'can_create_communities', label: 'Create communities', icon: Users, description: 'Allow creating new communities' },
    { key: 'video_uploads', label: 'Video uploads', icon: Video, description: 'Allow attaching videos to posts' },
    { key: 'verified_badge', label: 'Verified badge', icon: BadgeCheck, description: 'Show verified badge on profile' },
    { key: 'priority_support', label: 'Priority support', icon: Headphones, description: 'Access to priority support channel' },
    { key: 'early_access', label: 'Early access', icon: Rocket, description: 'Access to beta features' },
    { key: 'ads_enabled', label: 'Show ads / banners', icon: EyeOff, description: 'If true, users on this plan see ad banners in feed', inverse: true },
];

const NUMERIC_FEATURES = [
    { key: 'daily_post_limit', label: 'Daily post limit', icon: FileText, unit: 'posts/day' },
    { key: 'daily_comment_limit', label: 'Daily comment limit', icon: MessageSquare, unit: 'comments/day' },
    { key: 'max_communities_joined', label: 'Max communities joined', icon: Users, unit: 'communities' },
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 relative">
                                        <input
                                            ...
                                            className={`w-full pl-3 pr-16 py-1.5 text-xs border rounded-lg transition-all ${isUnlimited
                                    ...
                                    <button
                                        ...
                                        disabled={disabled}
                                        className={`flex-shrink-0 px-2 py-1 text-[10px] font-bold uppercase tracking-wide rounded border transition-colors ${isUnlimited
                                    ...
                                    >
                                        {isUnlimited ? <X size={10} className="inline" /> : <InfinityIcon size={10} className="inline" />}
                                    </button>
                                </div>
                            </div>
                        );
            ...
            <div className="p-3 bg-purple-50/50 dark:bg-purple-900/10 rounded-lg border border-purple-100 dark:border-purple-800/30">
                <div className="flex items-center gap-2 text-[11px] text-purple-700 dark:text-purple-300">
                    ...
                    <span className="font-medium">
                        {Object.keys(features).length} feature{Object.keys(features).length !== 1 && 's'} configured
                    </span>
                    <span className="text-gray-500 dark:text-gray-400">·</span>
                    <span className="text-gray-500 dark:text-gray-400">
                        Saved as JSON in the <code className="text-purple-600 dark:text-purple-400 bg-white/50 dark:bg-gray-800/50 px-1 rounded">features</code> column
                    </span>
                </div>
            </div>
        </div>
