// src/components/dashboard/subscriptions/SubscriptionsHeader.jsx
import React, { useState, useEffect, useRef } from 'react';
import { useTranslation } from 'react-i18next';
import { PackagePlus, Search, X, Filter, Grid, List, Download } from 'lucide-react';
  ...
  ({
    ...
    const handleExportData = async () => {
      const params = {
        ...
        subscription_status: selectedStatus !== 'all' ? selectedStatus : undefined,
        subscription_type: selectedType !== 'all' ? selectedType : undefined,
        sort_by: selectedSortBy,
        sort_order: selectedSortOrder,
      };

      try {
        const response = await subscriptionsApi.exportSubscriptions(params);
      ...
      <div className="p-4 border-b border-purple-100 dark:border-gray-700 transition-colors">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-3">
          <div>
            <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 transition-colors">
              {t('subscriptionsHeader.allSubscriptions')}
            </h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 transition-colors">
              {loading ? (
                ...
                t('subscriptionsHeader.totalSubscriptions', { count: subscriptionCount })
              )}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-purple-50 dark:bg-gray-700 rounded-lg p-0.5">
              <button
                ...
                title={t('subscriptionsHeader.listView')}
                disabled={loading}
              >
                <List
                  size={16}
              ...
              <button
                ...
                title={t('subscriptionsHeader.gridView')}
                disabled={loading}
              >
                <Grid
                  size={16}
            ...
            {onRefresh && (
              <RefreshButton
                onClick={onRefresh}
                loading={loading}
                title={t('subscriptionsHeader.refreshTitle')}
                size={18}
              />
            )}
            ...
            >
              <PackagePlus size={16} />
              <span>{t('subscriptionsHeader.addSubscription')}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
          ...
          <div className="relative flex-1 w-full sm:w-auto sm:min-w-[200px]">
            ...
            <input
              type="text"
              placeholder={t('subscriptionsHeader.searchPlaceholder')}
          ...
          <div className="relative shrink-0">
            ...
            >
              <option value="all">{t('subscriptionsHeader.filterTypeAll')}</option>
              <option value="free">{t('subscriptionsHeader.planTypes.free')}</option>
              <option value="pro">{t('subscriptionsHeader.planTypes.pro')}</option>
              <option value="premium">{t('subscriptionsHeader.planTypes.premium')}</option>
            </select>
          </div>
          ...
          <div className="relative shrink-0">
            ...
            >
              <option value="all">{t('subscriptionsHeader.filterStatusAll')}</option>
          ...
          <div className="relative shrink-0">
            ...
            >
              <option value="created_at">{t('subscriptionsHeader.sortDate')}</option>
              <option value="subscription_name">{t('subscriptionsHeader.sortName')}</option>
              <option value="subscription_price">{t('subscriptionsHeader.sortPrice')}</option>
            </select>
          </div>
          ...
          <button
            ...
            title={
              selectedSortOrder === 'DESC'
                ? t('subscriptionsHeader.sortTitleNewest')
                : t('subscriptionsHeader.sortTitleOldest')
            }
          >
            ...
            <span className="hidden sm:inline">
              {selectedSortOrder === 'DESC'
                ? t('subscriptionsHeader.sortDesc')
                : t('subscriptionsHeader.sortAsc')}
            </span>
          </button>
          ...
          <button
            ...
            title={
              hasActiveFilters
                ? t('subscriptionsHeader.clearAllFiltersTitle')
                : t('subscriptionsHeader.noFiltersTitle')
            }
          >
            <X size={14} />
            <span>{t('subscriptionsHeader.clear')}</span>
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {localSearchTerm && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('subscriptionsHeader.activeFilterSearch')}</span>
                ...
                <button
                  ...
                  title={t('subscriptionsHeader.removeSearchTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {selectedType !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('subscriptionsHeader.activeFilterType')}</span>
                ...
                <button
                  ...
                  className="text-purple-500 hover:text-purple-700 dark:hover:text-purple-200 ms-0.5 transition-colors"
                  title={t('subscriptionsHeader.removeTypeFilterTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {selectedStatus !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('subscriptionsHeader.activeFilterStatus')}</span>
                ...
                <button
                  ...
                  className="text-purple-500 hover:text-purple-700 dark:hover:text-purple-200 ms-0.5 transition-colors"
                  title={t('subscriptionsHeader.removeStatusFilterTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {(selectedSortBy !== 'created_at' || selectedSortOrder !== 'DESC') && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('subscriptionsHeader.activeFilterSort')}</span>
