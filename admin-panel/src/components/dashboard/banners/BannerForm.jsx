...
import {
    Image as ImageIcon, Link as LinkIcon, Globe, Hash,
    CreditCard, Calendar, Layers, ToggleLeft, AlertCircle,
} from 'lucide-react';
...
const BannerForm = ({
    ...
    const csvToArray = (csv) => {
        ...
        return csv.split(',').map(v => v.trim()).filter(Boolean).map(v => {
            const n = Number(v);
    ...
    const targetTopicArr = useMemo(() => csvToArray(formData.target_topic_ids), [formData.target_topic_ids]);
    const excludedTopicArr = useMemo(() => csvToArray(formData.excluded_topic_ids), [formData.excluded_topic_ids]);
    const targetSubArr = useMemo(() => csvToArray(formData.target_subscription_ids), [formData.target_subscription_ids]);
    const excludedSubArr = useMemo(() => csvToArray(formData.excluded_subscription_ids), [formData.excluded_subscription_ids]);
    ...
    const imagePreview = formData.banner_image instanceof File
    ...
    return (
        <form id={formId} onSubmit={onSubmit} className="space-y-5">
            ...
            <div>
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Recommended: 1200×400px. Max 10MB.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Display Order</label>
                    <input
                        type="number"
                        name="display_order"
                        min="0"
            ...
            <div className="pt-3 border-t border-purple-100 dark:border-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Valid From</label>
                        <input
                            type="datetime-local"
                            name="valid_from"
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Valid Until</label>
                        <input
                            type="datetime-local"
                            name="valid_until"
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Leave empty for no start/end date.</p>
            </div>
            ...
            <div className="pt-3 border-t border-purple-100 dark:border-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Countries</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Topics</label>
                        <MultiSelectField
                            options={topics}
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Subscriptions</label>
                        <MultiSelectField
                            options={subscriptions}
            ...
            <div className="pt-3 border-t border-purple-100 dark:border-gray-700">
                <div className="flex items-center gap-3">
                    ...
                    >
                        <span className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${formData.is_active ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${formData.is_active
                        ? 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30'
                        : 'text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700'}`}>
                        <span className={`w-1 h-1 rounded-full mr-1.5 ${formData.is_active ? 'bg-green-500' : 'bg-gray-400'}`} />
