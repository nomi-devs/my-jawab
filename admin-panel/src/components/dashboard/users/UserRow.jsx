...
const UserRow = React.memo(({ user, onDelete, onEdit, onViewDetails, index = 0, serialNumber = 0 }) => {
  const handleDelete = () => {
    onDelete(user.id);
  };
  ...
  return (
    ...
    >
      ...
      <td className="p-2 transition-colors">
        <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-semibold inline-flex items-center w-fit space-x-1 ${
          user.role === 'Pro' 
      ...
      <td className="p-2">
        <span className={`inline-flex items-center w-fit px-1.5 py-0.5 rounded-full text-[10px] font-medium transition-colors ${
          user.status === 'Active' 
          ...
          <span className={`w-1 h-1 rounded-full mr-1 ${
            user.status === 'Active' ? 'bg-green-500 dark:bg-green-400' 
            : user.status === 'Suspended' ? 'bg-red-500 dark:bg-red-400'
            : user.status === 'Pending' ? 'bg-yellow-500 dark:bg-yellow-400'
