  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const transformedNotifications = (response.data.data || []).map((notification) => ({
        id: notification.id,
        title: notification.title || t('defaultNotificationTitle'),
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500 ms-2 whitespace-nowrap">
                        {notification.time}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                  ...
                  <button
                    ...
                    className="ms-3 text-xs text-gray-400 hover:text-red-500 dark:hover:text-red-400"
                  >
                    {t('common:delete')}
                  </button>
                </div>
