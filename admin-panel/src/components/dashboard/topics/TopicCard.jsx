...
import { Hash, TrendingUp, TrendingDown, Users, MessageSquare, BarChart, Eye, Edit, Trash2 } from 'lucide-react';
...
const TopicCard = React.memo(({
  ...
  const mockTopic = {
    ...
    description: 'Discussions about AI advancements, machine learning models, and their impact on society and technology.',
  ...
  useEffect(() => {
    if (!propTopic) { // Only use mock data if no prop is provided
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-start space-x-2 sm:space-x-3 min-w-0 flex-1">
          ...
          <div className={`p-2 sm:p-3 rounded-xl bg-purple-500 relative flex-shrink-0 ${topic.topic_image && (!topic.parent_id || topic.parent_id === 0) ? 'hidden' : 'flex items-center justify-center'}`}>
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            <div className="flex items-center space-x-1.5 sm:space-x-2 mb-1 flex-wrap">
              <div className="flex items-center space-x-1 sm:space-x-1.5 flex-1 min-w-0">
                <h3 className="font-bold text-gray-800 dark:text-gray-100 truncate text-sm sm:text-base">{topic.topic_name || topic.name || 'Untitled Topic'}</h3>
              </div>
              ...
              <div className="flex items-center space-x-1 flex-shrink-0 mt-1 sm:mt-0">
                ...
                {(typeof topic.parent_id !== "undefined" && topic.parent_id !== null && Number(topic.parent_id) !== 0) && (
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-2 sm:p-3 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">
          <div className="flex items-center justify-between mb-1">
            <MessageSquare size={14} className="text-green-500 dark:text-green-400 sm:w-4 sm:h-4" />
            <span className="text-xs text-gray-500 dark:text-gray-400">Slug</span>
          </div>
          <div className="text-xs font-semibold text-gray-800 dark:text-gray-100 truncate">{topic.topic_slug || 'N/A'}</div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between text-sm text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 space-y-2 sm:space-y-0">
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <div className="flex items-center">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 mr-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <span className="text-xs whitespace-nowrap truncate">
              Created: {formatDate(topic.created_at || topic.createdAt || new Date().toISOString())}
            </span>
          </div>
          {topic.updated_at && (
            <div className="flex items-center">
      ...
      <div className="border-t border-purple-100 dark:border-gray-700 pt-3 sm:pt-4" onClick={(e) => e.stopPropagation()}>
