    ...
    >
      ...
      <td className="p-2 transition-colors">
        <div className="flex items-center space-x-2">
          ...
          <div className={`w-8 h-8 rounded-lg bg-purple-500 flex items-center justify-center text-white flex-shrink-0 ${topic.topic_image && (!topic.parent_id || topic.parent_id === 0) ? 'hidden' : ''}`}>
            <Hash size={14} />
          </div>
      ...
      <td className="p-2 transition-colors">
        <div className="flex flex-col gap-0.5">
          <span className={`inline-flex items-center w-fit px-1.5 py-0.5 rounded-full text-[10px] font-medium transition-colors ${topic.is_active
              ? 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30'
              : 'text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700'
            }`}>
            <span className={`w-1 h-1 rounded-full mr-1 ${topic.is_active ? 'bg-green-500 dark:text-green-400' : 'bg-gray-500 dark:bg-gray-400'
              }`}></span>
