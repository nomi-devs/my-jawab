...
const PostRow = React.memo(({
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-flex items-center w-fit px-1.5 py-0.5 rounded-full text-[10px] font-medium transition-colors ${config.className}`}>
      ...
      <td className="p-2 transition-colors">
        <div className="flex items-center space-x-3 text-[10px] text-gray-500 dark:text-gray-400">
          <div className="flex items-center space-x-1" title="Likes">
            <Heart size={10} className={post.stats?.likes > 0 ? "text-red-500 fill-red-500" : ""} />
            <span>{(post.stats?.likes || 0).toLocaleString()}</span>
          </div>
          <div className="flex items-center space-x-1" title="Dislikes">
            <ThumbsDown size={10} className={post.stats?.dislikes > 0 ? "text-amber-600 fill-amber-600" : ""} />
          ...
          <div className="text-[10px]">
            {(post.stats?.views || 0) >= 1000
              ...
              : (post.stats?.views || 0).toLocaleString()} views
          </div>
        </div>
      </td>
