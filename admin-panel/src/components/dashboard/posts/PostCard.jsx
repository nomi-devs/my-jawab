...
import { MessageCircle, Heart, ThumbsDown, Share2, Globe, Image as ImageIcon, Video, Lock, Clock, Eye, Edit, Trash2 } from 'lucide-react';
...
const PostCard = React.memo(({
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg border border-purple-100 dark:border-gray-700 overflow-hidden hover:shadow-md transition-all ${post.hidden ? 'opacity-60' : ''}`}>
      {/* Card Header */}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-purple-100 dark:border-gray-700">
          <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 transition-colors">
            <div className="flex items-center gap-1.5" title="Likes">
              <Heart size={14} className={post.stats.likes > 0 ? "text-red-500 fill-red-500" : ""} />
              <span>{(post.stats.likes || 0).toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-1.5" title="Dislikes">
              <ThumbsDown size={14} className={post.stats.dislikes > 0 ? "text-amber-600 fill-amber-600" : ""} />
          ...
          <div className="flex items-center gap-2">
            {post.post_status && (
              <span className={`px-2 py-0.5 text-xs rounded-full ${post.post_status === 'published'
