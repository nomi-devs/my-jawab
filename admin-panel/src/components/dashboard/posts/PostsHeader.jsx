// src/components/dashboard/posts/PostsHeader.jsx
import React, { useState, useEffect, useRef } from 'react';
import { useTranslation } from 'react-i18next';
import { FilePlus, Search, X, Filter, Grid, List, Download } from 'lucide-react';
import RefreshButton from '../../common/RefreshButton';
import ExportButton from '../../common/ExportButton';
import postsApi from '../../../api/postsApi';

const PostsHeader = React.memo(
  ({
    postCount,
    onAddClick,
    onSearch,
    ...
    const [selectedFeatured, setSelectedFeatured] = useState(featuredFilter);
    ...
    useEffect(() => {
      setSelectedFeatured(featuredFilter);
    }, [featuredFilter]);
    ...
    const clearFilters = () => {
      ...
      if (onFeaturedFilter) onFeaturedFilter('all');
    ...
    const handleExportData = async () => {
      const params = {
        ...
        post_status: selectedStatus !== 'all' ? selectedStatus : undefined,
        is_featured:
          selectedFeatured === 'featured'
            ? true
            : selectedFeatured === 'not_featured'
              ? false
              : undefined,
      ...
      try {
        const response = await postsApi.exportPosts(params);
      ...
      <div className="p-4 border-b border-purple-100 dark:border-gray-700 transition-colors">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 transition-colors">
              {loading ? (
                ...
                t('postsHeader.totalPosts', { count: postCount })
              )}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-purple-50 dark:bg-gray-700 rounded-lg p-0.5">
              <button
                ...
                title={t('postsHeader.listViewTitle')}
                disabled={loading}
              >
                <List
                  size={16}
              ...
              <button
                ...
                title={t('postsHeader.gridViewTitle')}
                disabled={loading}
              >
                <Grid
                  size={16}
            ...
            {onRefresh && (
              <RefreshButton
                onClick={onRefresh}
                loading={loading}
                title={t('postsHeader.refreshTitle')}
                size={18}
              />
            )}

            {/* Export Button */}
            <ExportButton fetchData={handleExportData} filename="posts_export" disabled={loading} />
            ...
            >
              <FilePlus size={16} />
              <span>{t('postsHeader.addPostButton')}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
          ...
          <div className="relative flex-1 w-full sm:w-auto sm:min-w-[200px]">
            ...
            <input
              type="text"
              placeholder={t('postsHeader.searchPlaceholder')}
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              <option value="all">{t('postsHeader.statusFilter.all')}</option>
              <option value="published">{t('postsHeader.statusFilter.published')}</option>
              <option value="draft">{t('postsHeader.statusFilter.draft')}</option>
              <option value="archived">{t('postsHeader.statusFilter.archived')}</option>
            </select>
          </div>
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              <option value="all">{t('postsHeader.featuredFilter.all')}</option>
              <option value="featured">{t('postsHeader.featuredFilter.featured')}</option>
              <option value="not_featured">{t('postsHeader.featuredFilter.notFeatured')}</option>
            </select>
          </div>
          ...
          <div className="relative flex-shrink-0">
            ...
            >
              <option value="created_at">{t('postsHeader.sortByOptions.date')}</option>
              <option value="like_count">{t('postsHeader.sortByOptions.likes')}</option>
              <option value="comment_count">{t('postsHeader.sortByOptions.comments')}</option>
              <option value="view_count">{t('postsHeader.sortByOptions.views')}</option>
              <option value="post_title">{t('postsHeader.sortByOptions.title')}</option>
            </select>
          </div>
          ...
          <button
            ...
            title={
              selectedSortOrder === 'DESC'
                ? t('postsHeader.sortNewestFirst')
                : t('postsHeader.sortOldestFirst')
            }
          >
            <span className="text-base leading-none rtl:rotate-180">
              {selectedSortOrder === 'DESC' ? '↓' : '↑'}
            </span>
            <span className="hidden sm:inline">
              {selectedSortOrder === 'DESC'
                ? t('postsHeader.descLabel')
                : t('postsHeader.ascLabel')}
            </span>
          </button>
          ...
          <button
            ...
            title={
              hasActiveFilters
                ? t('postsHeader.clearFiltersTitleActive')
                : t('postsHeader.clearFiltersTitleInactive')
            }
          >
            <X size={14} />
            <span>{t('postsHeader.clearButton')}</span>
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {localSearchTerm && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('postsHeader.searchBadgeLabel')}</span>
                ...
                <button
                  ...
                  title={t('postsHeader.removeSearchTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {selectedStatus !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('postsHeader.statusBadgeLabel')}</span>
                ...
                <button
                  ...
                  className="text-purple-500 hover:text-purple-700 dark:hover:text-purple-200 ms-0.5 transition-colors"
                  title={t('postsHeader.removeStatusTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {selectedFeatured !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('postsHeader.featuredBadgeLabel')}</span>
                <span className="capitalize">{selectedFeatured.replace('_', ' ')}</span>
                <button
                  onClick={() => {
                    setSelectedFeatured('all');
                    if (onFeaturedFilter) onFeaturedFilter('all');
                  ...
                  className="text-purple-500 hover:text-purple-700 dark:hover:text-purple-200 ms-0.5 transition-colors"
                  title={t('postsHeader.removeFeaturedTitle')}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {(selectedSortBy !== 'created_at' || selectedSortOrder !== 'DESC') && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
                <span className="font-medium">{t('postsHeader.sortBadgeLabel')}</span>
