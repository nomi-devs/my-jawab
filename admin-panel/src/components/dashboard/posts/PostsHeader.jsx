...
const PostsHeader = React.memo(({
  ...
  const hasActiveFilters = localSearchTerm !== '' || selectedStatus !== "all" || selectedFeatured !== "all" || selectedSortBy !== "created_at" || selectedSortOrder !== "DESC";

  const handleExportData = async () => {
    const params = {
      search: localSearchTerm,
      post_status: selectedStatus !== 'all' ? selectedStatus : undefined,
      is_featured: selectedFeatured === 'featured' ? true : (selectedFeatured === 'not_featured' ? false : undefined),
  ...
  return (
    <div className="p-4 border-b border-purple-100 dark:border-gray-700 transition-colors">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-3">
        <div>
          <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 transition-colors">All Posts</h3>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-purple-50 dark:bg-gray-700 rounded-lg p-0.5">
            <button
              ...
              disabled={loading}
            >
              <List size={16} className={viewMode === 'list' ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500'} />
            </button>
            <button
              ...
              disabled={loading}
            >
              <Grid size={16} className={viewMode === 'grid' ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500'} />
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
        ...
        <div className="relative flex-shrink-0">
          ...
          >
            ...
            <option value="post_title">Sort: Title</option>
          </select>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${selectedSortOrder === 'DESC'
      ...
      <div className={`mt-2 overflow-hidden transition-all duration-300 ease-in-out ${hasActiveFilters ? 'max-h-32 opacity-100 mt-2' : 'max-h-0 opacity-0 mt-0'
        }`}>
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {(selectedSortBy !== "created_at" || selectedSortOrder !== "DESC") && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-xs text-purple-700 dark:text-purple-300 animate-in fade-in slide-in-from-left-2 duration-200">
              ...
              <span className="capitalize">{selectedSortBy.replace('_', ' ')} {selectedSortOrder}</span>
            </span>
          )}
        </div>
      </div>
