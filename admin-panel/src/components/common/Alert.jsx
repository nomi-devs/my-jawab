...
const Alert = React.memo(({ 
  ...
  return (
    <div className={`fixed ${positionClasses[position]} z-[9999] animate-in slide-in-from-right duration-300 max-w-sm w-full`}>
      <div className={`${style.bg} ${style.border} border rounded-lg p-3 md:p-4 shadow-lg flex items-start space-x-3 transition-colors`}>
        <div className="flex-shrink-0 mt-0.5">
          {style.icon}
        </div>
        <div className="flex-1 min-w-0">
          <p className={`${style.text} text-sm md:text-base font-medium break-words transition-colors`}>
            {message}
          </p>
        </div>
        {onClose && (
