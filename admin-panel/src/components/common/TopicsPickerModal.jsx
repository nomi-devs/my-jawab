    ...
    ({ isOpen, onClose, initialSelectedIds = [], onSave, multiple = true }) => {
        ...
        const filteredTopics = useMemo(() => {
            ...
            return topics
                .map((parent) => {
                    const parentMatches =
                        parent.name && parent.name.toLowerCase().includes(lower);
                    const matchingChildren =
                        parent.children?.filter(
                            (child) =>
                                child.name && child.name.toLowerCase().includes(lower)
                        ) || [];
        ...
        return (
            <div className="fixed inset-0 z-[100] bg-black/50 dark:bg-black/70 flex items-center justify-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col border border-purple-100 dark:border-gray-700 overflow-hidden">
                    ...
                    {/* Body */}
                    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3" style={{ scrollbarGutter: 'stable' }}>
                        {loading && (
                        ...
                        {!loading && !error && filteredTopics.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-8 text-center">
                                <span className="text-2xl mb-2 text-gray-300 dark:text-gray-600">
                                    #
                                </span>
                                                    ...
                                                    {parent.children.map((child) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`px-3 py-1 rounded-full border text-[11px] flex items-center gap-1 transition-all ${isSelected
