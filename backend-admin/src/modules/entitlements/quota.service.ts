...
export class QuotaService {
     ...
     */
    async getUsed(userId: number, featureKey: FeatureKey | string): Promise<number> {
     ...
     */
    async status(userId: number, featureKey: FeatureKey | string): Promise<QuotaStatus> {
     ...
     */
    async canConsume(userId: number, featureKey: FeatureKey | string): Promise<boolean> {
    ...
    async consume(
        ...
        if (newCount > limit) {
            ...
            await this.redis.execute(client => client.decr(this.buildKey(userId, featureKey)));

            throw new ForbiddenException({
                statusCode: 403,
                error: 'Quota Exceeded',
                message: errorMessage || `Daily limit reached for ${featureKey} (${limit}/day). Resets at midnight UTC.`,
                upgrade_required: true,
                feature: String(featureKey),
                quota: {
                    limit,
     ...
     */
    private async incrementCounter(userId: number, featureKey: string): Promise<number> {
        const key = this.buildKey(userId, featureKey);
    ...
    async getMultipleStatuses(
        ...
        const results = await Promise.all(
            featureKeys.map(async (key) => [key, await this.status(userId, key)] as const),
        );
